...
const AvailableBouquets = () => {
  ...
  const { data: items, refetch } = useGetAvailableBouquetByIdQuery(
    selectedBouquetId,
  )
                  ...
                  <Grid item key={bouquet.id}>
                    <List>
                      <AvailableBouquetsList
                        ...
                        changePrice={() =>
                          handleEditPrice(bouquet.id, bouquet.actual_price)
                        }
                        handleCancelClick={() => setEditingPriceId(null)}
                        isEditing={editingPriceId === bouquet.id}
                        handleSaveClick={(bouquetId, newPrice) =>
                          handleSaveClick(bouquetId, newPrice, bouquet.count)
                        }
                        editingPrice={editingPrice}
                        handlePriceChange={handlePriceChange}
                        sellBouquet={(id, totalSum, qty) =>
                          sellBouquet(id, totalSum, qty)
                        }
                      />
                    </List>
